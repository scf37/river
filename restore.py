...
def print_usage():
    ...
    print('Backups store absolute paths so files will be extracted to the same place where they were taken')
    print('to extract to specific path, use --to option: --restore <url> --to ~/tmp')
    print('If local configuration is unavailable, backup url can be constructed manually:')
